...
public:
    ...
    void createSystem( int which ) {
        ...
        if ( which == 1) {        
            ...
            // New Particles
            Particle* p3 = new Particle(400, 100, 0, 0);
            p3->index = particles.size();
            p3->color = { 2.0f, 0.5f, 1.0f };
            particles.push_back(p3);
            Particle* p4 = new Particle(400, 90, 0, 0);
            p4->index = particles.size();
            p4->color = { 1.0f, 0.5f, 0.0f };
            particles.push_back(p4);
            Particle* p5 = new Particle(400, 80, 0, 0);
            p5->index = particles.size();
            p5->color = { 0.5f, 1.0f, 1.0f };
            particles.push_back(p5);
            Particle* p6 = new Particle(400, 70, 0, 0);
            p6->index = particles.size();
            p6->color = { 1.0f, 1.0f, 0.0f };
            particles.push_back(p6);
            Particle* p7 = new Particle(320, 250, 0, 0);
            p7->index = particles.size();
            p7->color = { 0.5f, 0.5f, 0.5f };
            p7->mass = 500;
            particles.push_back(p7);
            Particle* p8 = new Particle(320, 50, 0, 0);
            p8->index = particles.size();
            p8->color = { 0.0f, 1.0f, 0.0f };
            p8->mass = 100;
            ...
            Spring* s1 = new Spring(p1, p3);
            s1->k = 0;
            Spring* s2 = new Spring(p1, p4);
            s2->k = 0;
            Spring* s3 = new Spring(p1, p5);
            s3->k = 0;
            Spring* s4 = new Spring(p1, p6);
            s4->k = 0;
            springs.push_back(new Spring(p1, p2));
    ...
    void derivs(float t, VectorXf& p, VectorXf& dpdt)
    {
        ...
        for (Particle* p : particles)
        {
            ...
            if (useGravity == true)
            {
                // Apply gravity only in the y direction
                p->f.y += p->mass * gravity;
            }

            // Add damping 
            p->f.x -= viscousDamping * p->v.x;
            p->f.y -= viscousDamping * p->v.y;
        }

        // Apply force on strings
