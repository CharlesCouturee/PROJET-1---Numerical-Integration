...
void mouse_pos_callback(GLFWwindow* window, double x, double y) {
    ...
    if (mouseDown)
    { // dragged
        particleSystem.particles[particleSystem.particles.size() - 1]->p[0] = xcurrent;
        particleSystem.particles[particleSystem.particles.size() - 1]->p[1] = ycurrent;

        for (Spring* s : particleSystem.particles[particleSystem.particles.size() - 1]->springs) {
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT) {
        ...
        if (pressed) {
            ...
            if (run)
            {
                if (mouse_p == NULL)
                {
                    ...
                    mouse_p = particleSystem.createParticle(xcurrent, ycurrent, 0, 0);

                    Spring* s = new Spring(mouse_p, p1);
                    mouse_string_p = p1;
                    particleSystem.springs.push_back(s);
                }
            }
        }
        ...
        if (released) 
        {
            ...
            if (run)
            {
                if (mouse_p != NULL)
                {
                    particleSystem.removeSpring(mouse_p, mouse_string_p);
                    mouse_string_p = NULL;

                    particleSystem.remove(mouse_p);
                    mouse_p = NULL;
...
int main(int argc, char** argv) {
    ...
    window = glfwCreateWindow(1280, 720, "COMP 559 W23 - A1 Particle System - CHARLES COUTURE", NULL, NULL);
    if (!window) {
        glfwTerminate();
        return -1;
    }
